...
[enclaves](intel-sgx-technology.md) and never leave them. The same thing applies
to secrets.
...
to push the encryption key to the [SMS](intel-sgx-technology.md) so it is
...
[SMS](intel-sgx-technology.md). Now we need to build the application that is
